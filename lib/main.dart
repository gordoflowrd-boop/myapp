...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _login() async {
    ...
    setState(() { _isLoading = true; _mensaje = "Validando..."; });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => MenuPage(userData: data['usuario'], token: data['token'])),
        );
      } else {
        setState(() {
                  ...
                  child: ListTile(
                    ...
                    onTap: () {
                      if (menuItems[index]['title'] == 'Venta') {
                        Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => VentaPage(userData: userData, token: token)),
                        );
                      }
                    },
                  ),
...
class _VentaPageState extends State<VentaPage> {
  ...
  final List<Map<String, dynamic>> _listaJugadas = [];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          if (_jornadas.isNotEmpty) _selectedJornadaId = _jornadas[0]['jornada_id'].toString();
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  void _agregarJugada() {
    ...
    if (num.length == 2) {
      mod = "Q";
    } else if (num.length == 4) mod = "P";
    else if (num.length == 6) mod = "T";
    else return;

    setState(() {
      double monto = cant.toDouble(); 
      _listaJugadas.add({"modalidad": mod, "numeros": num, "cantidad": cant, "monto": monto});
      _totalTicket += monto;
  ...
  Future<void> _venderTicket() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || data['estado'] == 'ok') {
        ...
        setState(() { _listaJugadas.clear(); _totalTicket = 0; });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Venta SuperBett"), backgroundColor: Colors.blueGrey, foregroundColor: Colors.white),
      body: _isLoading 
        ...
        : Column(
            children: [
              Container(
                ...
                child: Text("${widget.userData['nombre']} — ${widget.userData['username']}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
              ),
              Padding(
                ...
                child: Column(
                  children: [
                      ...
                      children: [
                        Expanded(
                          ...
                          child: DropdownButtonFormField<String>(
                            ...
                            items: _jornadas.map((j) => DropdownMenuItem(value: j['jornada_id'].toString(), child: Text(j['nombre'], style: const TextStyle(fontSize: 12)))).toList(),
                            onChanged: (val) => setState(() => _selectedJornadaId = val),
                    ...
                    Row(
                      children: [
                        Expanded(child: TextField(controller: _numController, keyboardType: TextInputType.number, decoration: const InputDecoration(hintText: "Jugada", border: OutlineInputBorder()))),
                        const SizedBox(width: 5),
                        Expanded(child: TextField(controller: _cantController, keyboardType: TextInputType.number, decoration: const InputDecoration(hintText: "Cant", border: OutlineInputBorder()))),
                        const SizedBox(width: 5),
                        ElevatedButton(onPressed: _agregarJugada, style: ElevatedButton.styleFrom(backgroundColor: Colors.blueGrey, foregroundColor: Colors.white, padding: const EdgeInsets.all(18)), child: const Text("ADD")),
                      ],
                    ),
                  ],
                ),
              ...
              Expanded(
                child: GridView.builder(
                  padding: const EdgeInsets.all(10),
                  gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, childAspectRatio: 1.4, crossAxisSpacing: 8, mainAxisSpacing: 8),
                ...
                child: Row(
                  children: [
                    Expanded(child: ElevatedButton(
                      ...
                      child: _isSending ? const CircularProgressIndicator(color: Colors.white) : const Text("VENDER", style: TextStyle(fontWeight: FontWeight.bold))
                    )),
                    const SizedBox(width: 10),
                    Expanded(child: ElevatedButton(onPressed: () => setState(() { _listaJugadas.clear(); _totalTicket = 0; }), style: ElevatedButton.styleFrom(backgroundColor: Colors.orange, foregroundColor: Colors.white, padding: const EdgeInsets.all(15)), child: const Text("LIMPIAR"))),
                  ],
                ),
              )
            ],
